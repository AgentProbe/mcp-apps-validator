...
| Package | `mcp-apps-validator` v0.1.0, `type: module`, `main: ./src/index.mjs` |
...
|--------|-----------|---------|
| `start` | `{ endpoint, timeout = 10000 }` | `{ status, messages, categories, entries }` |
...
categories and reports the reason in `messages` (codes `CON-*`, `UIR-*`, `UIV-*`).
